...
function preprocessMd(md: string): string {
  ...
  // Normalize for matching: lowercase, replace non-alphanumeric (except -) with -, trim
  ...
  clean = clean.replace(/\[\[([^\]]+?)\]\]/g, (match, name) => {
    ...
    if (found) return `<a href="${getFileUrl(found.id, found.section)}" class="internal-link">${name.trim()}</a>`;
    return match;
  });
  return clean;
}
